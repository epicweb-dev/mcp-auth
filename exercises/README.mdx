...
[Model Context Protocol (MCP)](https://modelcontextprotocol.io/specification/2025-11-25) servers—the essential skills for building secure, production-ready MCP applications that handle real user data.
...
<callout-info>
	In OAuth terms, an authenticated MCP server is the OAuth **Protected
	Resource** (or Resource Server). It accepts access tokens, validates them, and
	enforces resource-specific authorization. The Authorization Server handles
	user login, consent, token issuance, and token metadata.
</callout-info>

<callout-warning>
	Do not pass third-party access tokens, API keys, or other service credentials
	through the MCP client to your MCP server. Use a proper server-side OAuth
	integration, or URL mode elicitation for out-of-band account connection flows
	where the user interacts directly with the trusted service.
</callout-warning>

<callout-info>
	For Streamable HTTP deployments that validate the `Origin` header, reject
	invalid origins with `403 Forbidden`. Keep that distinct from authentication
	failures, where missing or invalid bearer tokens should produce `401
