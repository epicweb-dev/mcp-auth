...
- Include enough auth challenge metadata for clients to discover the protected
  resource metadata and recover gracefully
...
```
WWW-Authenticate: Bearer realm="example", error="invalid_token", error_description="The access token is invalid", resource_metadata="https://example.com/.well-known/oauth-protected-resource/mcp"
```
...
`resource_metadata` is optional when clients can discover your
`/.well-known/oauth-protected-resource/...` endpoint themselves, but including it
in the challenge points clients directly at the right metadata. Missing or
invalid tokens should use `401 Unauthorized`; valid tokens with insufficient
resource-specific scopes should use `403 Forbidden` with
`error="insufficient_scope"`.

Clients should not always perform Dynamic Client Registration. In open
ecosystems, OAuth Client ID Metadata Documents are the preferred path when the
authorization server advertises support. Treat DCR as a fallback, and remember
that public clients normally use `token_endpoint_auth_method: "none"` rather
...
- Keep auth params quoted, including `resource_metadata="..."`
