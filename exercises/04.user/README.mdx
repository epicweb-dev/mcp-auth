...
User identity must come from validated token claims or trusted introspection
results. Do not derive the authenticated user from elicitation responses,
tool arguments, headers supplied by the client, or other user-controlled request
fields.
...
<callout-info>
	If you later use URL mode elicitation for connecting a third-party account,
	bind that out-of-band authorization flow to the already-authenticated MCP
	user/session and verify the same user completes it before storing credentials
	or linking accounts.
</callout-info>
